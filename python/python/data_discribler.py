...
from data_loader import nan_solver
...
def feature_stater(data,colname,dtype="continuous",stat="means",NaNaction="n",p=1,**kwargs):
    nan_solver(data,colname,NaNaction,kwargs)
...
def draw_ctn(data,colname,stat="means",p=1,**kwargs):
    ...
    if stat.lower()=="means":
        ...
        if p:
            print(d)
            #print(ggplot(data, aes(x=colname)) + geom_bar(stat="identity"))
        return d
...
def feature_discribler(data,xlabel,ylabel=0,xtype="continuous",ytype="continuous",stat="means",NaNaction="n",p=1,**kwargs):
    nan_solver(data,[xlabel,ylabel],[NaNaction,NaNaction],kwargs)
    if data.__class__==list:
        data=pd.DataFrame(data)
    if NaNaction.lower()=="n":
        #查看两个变量的基本情况。
...
'''
对数据表进行描述
@data 数据表:pd.DataFrame
@head 展示前head行
@tail 展示后tail行
@corr_method 相关系数矩阵计算方法:"pearson","kendall","spearman",默认为"pearson"

'''
def data_discribler(data,head=5,tail=5,corr_method="pearson",NaNaction="N",**kwargs):
    nan_solver(data,data.columns,[NaNaction for i in data.columns],kwargs)
